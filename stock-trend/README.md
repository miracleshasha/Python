...
### (선택) 한국투자증권(KIS) Open API — 실시간·분봉·수급

[KIS Developers](https://apiportal.koreainvestment.com)에서 앱키/시크릿을 발급(무료)해 환경변수로 지정하면:

```bash
export KIS_APP_KEY="발급받은_앱키"
export KIS_APP_SECRET="발급받은_앱시크릿"
# export KIS_ENV="real"   # real(실전, 기본) | vps(모의)
```

- 추세판정 페이지에 **실시간 현재가·당일 분봉**이 추가됩니다.
- 예측 랭킹의 **수급(외국인·기관)을 KIS로 조회**해 **클라우드(해외 서버)에서도 동작**합니다.
- 키가 없으면 수급은 pykrx(국내망)로 폴백하고, 실시간/분봉 섹션은 표시되지 않습니다.
- 조회(시세·수급)만 사용하며 **주문/자동매매 기능은 없습니다.**
...
- **⚡ 실시간·분봉 (KIS)** — 한국투자증권 Open API 키가 있으면 추세판정 페이지에 실시간 현재가·당일 분봉 표시,
  예측 랭킹의 **수급(외국인·기관)도 KIS로 조회해 클라우드에서 동작**
